...
export interface TicketHubTemplate {
  ...
  name: string
  description: string
  category: 'Tickets' | 'Estimates' | 'Invoices' | 'Reminders' | 'Appointments' | 'CSAT'
...
const TEMPLATES: TicketHubTemplate[] = [
  ...
  {
    mode: 'CSAT_SURVEY',
    name: 'CSAT survey · post-resolution',
    description:
      "Sent automatically the first time a ticket transitions to RESOLVED or CLOSED. Carries a one-time token-authenticated link to /csat/[token] where the contact picks 1–5 stars and optionally leaves a comment. Gated by TICKETHUB_CSAT_SURVEY_ENABLED env. One survey per ticket lifetime (re-opens don't trigger a second).",
    category: 'CSAT',
    source: 'app/lib/csat-survey.ts → sendCsatSurvey',
    logged: true,
  },
]
